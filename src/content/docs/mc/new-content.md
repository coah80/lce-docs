...
MinecraftConsoles adds several blocks and items that are not present in the LCEMP base. This page documents each addition based on the actual source code.
...
Extends `RotatedPillarTile`. A directional block with top and side textures, like logs.
...
Extends `BaseEntityTile`. A tile entity that plays musical notes with redstone interaction.
...
- **Note rendering** (`triggerEvent`): Calculates pitch as `pow(2, (note - 12) / 12.0)` and selects an instrument sound based on the block below (harp, bass drum, snare, hat, bass). Spawns a `note` particle.
...
- **Right-click**: Ejects the current disc if one is present (data value `1` indicates occupied).
...
Allows players to tie leashed mobs to fence posts.
...
When used on a fence tile, searches a 7-block radius for mobs leashed to the player, creates or finds a `LeashFenceKnotEntity` at that position, and attaches them. Has a `bindPlayerMobsTest()` method for tooltip prediction without side effects.
...
When the item has a custom hover name and is used on a `Mob`, sets the mob's custom name and calls `setPersistenceRequired()` to prevent despawning. Decrements the stack.
...
Creates a new `ItemInstance` of `Item::map` with aux value `-1`, calls `Item::map->onCraftedBy()` to initialize it, then decrements the empty map stack. If the player's stack is not empty afterward, adds the new map to inventory or drops it.
...
`spawnMobAt()` includes a 4J-added `piResult` parameter for communicating spawn failures back to the dispenser system.
...
Used for items like the Nether Star that should always appear enchanted.
...
For more details on fireworks specifically, see the [Fireworks](/lcemp-docs/mc/fireworks/) page. For horse-related items (armor, saddles), see the [Horses](/lcemp-docs/mc/horses/) page.
