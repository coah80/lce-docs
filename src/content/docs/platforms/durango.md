...
The Xbox One port (codenamed "Durango") lives in `Minecraft.Client/Durango/`. It represents a significant evolution from the Xbox 360 version, moving from QNET to Xbox Realtime Networking Sessions (XRNS) and from XUI to the Iggy UI framework.
...
- **Save incomplete handling**: `Callback_SaveGameIncomplete` with message box for insufficient storage
...
This differs from Windows 64 which uses raw pointers, reflecting Xbox One's WinRT/COM-based API.
...
Three layers:
1. **`CPlatformNetworkManagerDurango`** -- Game-facing interface implementing `CPlatformNetworkManager`
2. **`DQRNetworkManager`** -- Low-level XRNS session management
...
- **Party integration**: `PartyController` for Xbox party system
- **Chat integration**: `ChatIntegrationLayer` for voice chat

### State Machine

Extensive internal state machine with states:
- `INITIALISING` -> `IDLE`
...
- `m_RTSMessageQueueIncoming` -- Events from XRNS to main thread
- `m_RTSMessageQueueOutgoing` -- Commands from main thread to XRNS
...
- `FriendPartyManagerSearch` enumerates friend parties
...
`AchievementManager` in the `Achievements/` subdirectory manages Xbox One achievements, which use the newer event-based system rather than Xbox 360's direct achievement APIs.
...
- **Secure networking**: All peer-to-peer traffic encrypted via Xbox Secure Device Associations
- **Party-first networking**: Players join via Xbox party system rather than direct session search
