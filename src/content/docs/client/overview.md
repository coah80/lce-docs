...
The `Minecraft.Client` module contains all client-side code for Legacy Console Edition Minecraft: rendering, UI, input handling, audio, texture management, networking stubs, and platform abstraction. It depends on `Minecraft.World` for game logic (entities, tiles, levels, items) and provides the visual and interactive layer that players see and interact with.

## Module entry point

The central class is `Minecraft` (declared in `Minecraft.h`). It owns references to every major subsystem:
...
LCEMP supports up to four local players through split-screen. The `Minecraft` class maintains per-player arrays for many subsystems:
...
Methods like `addLocalPlayer()`, `createExtraLocalPlayer()`, `removeLocalPlayerIdx()`, and `updatePlayerViewportAssignments()` manage the lifecycle of additional split-screen players. The `localPlayerIdx` field tracks which player viewport is currently being rendered.

## Game loop

The game loop is split into three phases to integrate with the console platform loop:

1. **`run()`** -- entry point, calls `run_middle()` and `run_end()` in sequence
2. **`run_middle()`** -- processes input, ticks game logic, renders frames
3. **`run_end()`** -- cleanup and frame timing

The `tick()` method drives per-tick updates. It accepts `bFirst` (true for the first active viewport) and `bUpdateTextures` (true when texture animations should advance). This separation lets split-screen viewports share texture tick work.

## Level management

The client can hold multiple levels simultaneously (one per dimension: Overworld, Nether, End):
...
- `Entity.h`, `Level.h`, `Vec3.h` -- core game objects
- `LevelListener.h` -- `LevelRenderer` implements this interface to receive world change events
- `ProgressListener.h` -- `ProgressRenderer` implements this for loading progress callbacks
- `DisconnectPacket.h` -- disconnect reason enumeration for multiplayer
- `C4JThread.h` -- threading primitives shared across both modules

## Common/ subdirectory

The `Common/` subdirectory contains platform-shared code organized into these areas:
...
The `Minecraft` class also exposes static helpers:
...
- `Minecraft::maxSupportedTextureSize()` -- platform texture size limit
